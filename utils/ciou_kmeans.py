...
def ciou_distance(box, anchor):
    """
    CIOU distance calculation per paper.
    Centers are aligned, so ρ²(bA, bB) / c² = 0
    Returns 1 - CIOU which serves as the distance measure.
    """
    w1, h1 = box
    w2, h2 = anchor
...
def ciou_kmeans(boxes, k, dist_fn=calc_dist_matrix, max_iter=300):
    """
    K-means using CIOU distance.
    boxes: numpy array of shape (N, 2)
    k: number of clusters (e.g., 12 for 4 scales x 3 anchors)
    """
